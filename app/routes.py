...
def setup_db():
	...
	db.child('user_data').child(session['user']).set(data)
...
def profile():
	if session['user'] == False or session['logged'] == False:
		return redirect(url_for('index'))
	userdata = db.child('user_data').child(str(session['user'])).get()
...
@app.route('/user')
def user():
	userdata = db.child('user_data').child(str(session['visit'])).get()
