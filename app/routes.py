...
def add_user():
	...
	db.child('users').child(timestamp).set({
		...
		'data' : {
			...
			'bio' : '',
			'link' : 'websitename/profile/'+request.form.get('username')
		}, 
...
def setup_db():
	...
	db.child('users').child(fsession['user']).child('data').update(data)
...
def profile():
	if fsession['user'] == False or session['logged'] == False:
		return redirect(url_for('index'))
	userdata = db.child('users').child(str(fsession['user'])).child('data').get()
...
def update_db():
	...
	db.child('users').child(str(fsession['user'])).child('data').update(data)
...
@app.route('/user')
def user():
	userdata = db.child('users').child(str(fsession['visit'])).child('data').get()
